       ...
       <>
        ...
        {currState === 'signup'&& isDataSubmitted  &&(
           <textarea rows={4} onChange={(e)=>setBio(e.target.value)} value={bio} 
            placeholder='provide a Bio' required className='p-2 border border-gray-500 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500'/>
        )}


        <button  type='submit' className='bg-gradient-to-r from-purple-400 to-violet-600 py-3 text-white rounded-md cursor-pointer'>
          {currState === 'signup' ? 'Create Account' : 'Login Now'}
        </button>

        <div className='flex items-center gap-2 text-xm text-gray-500'>
          <input type="checkbox" />
